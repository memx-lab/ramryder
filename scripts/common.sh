...
function allocate_memory_object() {
    local node_id=$1 vm_id=$2 size_mb=$3
    local response

    # format: resouece_client alloc-mem nid=<node id> vid=<VM id> size=<mb>
    response="$($RPC_CLIENT_BIN alloc-mem nid=$node_id vid=$vm_id size=$size_mb)"
